  ...
  return (
    ...
    >
      <div className={`flex max-w-[92%] flex-col gap-2 sm:max-w-[80%] ${isUser ? "items-end" : "items-start"}`}>
        <div className={`rounded-3xl border px-4 py-3 shadow-sm ${isUser ? "border-lime-600/20 bg-lime-600 text-white" : "border-stone-200 bg-surface text-stone-800"}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {message.choices.map((choice) => (
              <button
                ...
                className="rounded-full border border-stone-200 px-4 py-2 text-sm text-stone-700 transition hover:border-lime-500 hover:bg-lime-50 hover:text-lime-800"
              >
                {choice.label}
              </button>
            ))}
