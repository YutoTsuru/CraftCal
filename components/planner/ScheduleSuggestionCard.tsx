...
import type { PlannerSuggestion } from "@/lib/planner";
    ...
    >
      ...
      <div className="mt-4 grid gap-3">
        {suggestions.map((item) => (
          <div key={`${item.time}-${item.taskName}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-col gap-1">
              <p className="text-sm font-semibold text-slate-900">{item.time}</p>
              <p className="text-sm text-slate-800">{item.taskName}</p>
              <p className="text-xs text-slate-500">
                {item.projectName} / {item.estimatedMinutes}分 / {item.reason}
              </p>
            </div>
          </div>
        ))}
